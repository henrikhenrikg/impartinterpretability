...
%are useful tools to (redundant with next sentence?)
capture semantic and syntactic features
of
%language.
words.
They have been demonstrated to be useful in a broad range of
applications involving lexical semantics. In
neuroscience, they are employed to map
semantic representations across cortex
\enote{hs}{what does ``mapping across cortex'' mean?}
\citep{ruan16brainActivity,huth16semanticMaps}, as part of a
decoder that extracts linguistic meaning from measured brain
activity \citep{pereira18universalDecoder} and to detect
...
polarization in social media \citep{demszky19analyzing} can
be analyzed based on
word and
sentence embeddings
\citep{demszky19analyzing}. 
Evolutionary linguists track
historical changes in word meaning
with embeddings \citep{hamilton16diachronic}. Recent studies suggest
that embeddings  capture and
quantify  gender and ethnic
%stereotypes and (redundant?)
biases
in language
\citep{bolukbasi16debiasing,garg18gender100years,caliskan17humanLikeBiases}
and their evolution over time
\citep{agarwal2019word}. Embeddings can also 
quantify the semantic similarity between languages
\citep{senel17crossLingual,senel18atlas}.

\enote{hs}{what is the semantic semilarity between two alnguages?}
...
is their lack of interpretability: semantic concepts are not
represented by individual dimensions of the dense vector
space \citep{chen16InfoGAN,levy14dependency}.
\enote{hs}{previous sentence: check that i didn't change
  your intended meanign}
\enote{hs}{below: what does it mean for two word embewddings
  to make sense in relation to each other?}
Standard word embeddings make sense only in relation to each
other; individual dimensions cannot be interpreted.
Yet interpretability of word embeddings is highly desired
for several reasons. (i) Word embeddings serve
as base representation in many deep learning models,
so their interpretability is key for
...
(ii) The ability to interpret
an embedding model would unravel the semantic concepts
represented along the vector space dimensions.
(iii) In interpretable embeddings models, it is easier
to remove redundant or nonrelevant
dimensions, resulting in reduced computation and memory
requirements.
(iv) Interpretability 
also facilitates removal of
gender, race and other biases
\citep{dufter19ultraDense}.
...
\enote{hs}{below: ``presevering internal semantic strucutre'' is not
  defined. what does that mean? that similarities between
  words are (roughly) the same as before?}

In a recent study, the imparting approach was proposed to obtain
interpretable word embeddings while still preserving
internal semantic structure \citep{senel20impart}. The
...
investigated. \enote{hs}{bidirectionarlity is a clear
