...
of words. 
Although the remarkable performance of transformer based architectures \citep{vaswani17transformers,devlin19BERT,Radford19GPT2} on many NLP tasks moved the community towards the use contextualized embeddings, static word embeddings are still useful in a broad range of applications involving lexical semantics. 
In neuroscience, they are employed to analyse the representation of semantics in brain
activity
\enote{hs}{what does ``mapping across cortex'' mean?}
\enote{ks}{I changed the sentence, is it more clear now?}
...
\enote{hs}{is ``diagnosing schizophrenia'' really part of neuroscience?}
\enote{hs}{I changed it from neuroscience to psychiatry and behavioral science which hopefully makes more sense.}
...
\enote{hs}{what is the semantic similarity between two languages?}
\enote{ks}{Is it more clear now?}
...
for several reasons. 
(i) It will enable researchers to make sense of embeddings of individual words which are currently meaningful only in relation to other embeddings 
...
\enote{hs}{following sentence: ideally, you should say what
  the benefit of ``unraveling semantic concoepts'' is. it
  sounds a little bit like ``
``The ability to interpret
an embedding model would make the embeddings interpretable''
(i.e., it's a tautology?)}
\enote{ks}{I removed that sentence and added another sentence as first item, (i). Does this sentence make more sense or still sound like a tautology?}
...
\enote{hs}{below: ``presevering internal semantic strucutre'' is not defined. what does that mean? that similarities between words are (roughly) the same as before?}
\enote{ks}{Yes, I tried to clarify that}
In a recent study, the imparting approach was proposed to obtain
interpretable word embeddings while still keeping
the similarities between words mostly unchanged (i.e., semantic structure is preserved) \citep{senel20impart}. The
...
\enote{hs}{bidirectionarlity is a clear
  argument for novelty. ``generalizability of the imparting
  method
  beyond GloVe'' not so much -- sounds pretty weak. maybe
  add 3-4 weaker differentiatros as an enumeratino?}
\enote{ks}{I tried to divide the last paragraph into two paragraphs and structure them more fitting to ACL style. }
...
The proposed method is demonstrated on the skip-gram model of the word2vec method \citep{mikolov13word2vec_a,mikolov13word2vec_b}, where Roget's Thesaurus and \textit{WordNet} are leveraged to select concepts. 
...
Inspired by the results in \citep{bolukbasi16debiasing}, we also demonstrate that bidirectional imparting can concentrate gender information in a single embedding dimension as a continuum. 
As a result, it enables efficient capture of gender bias and debiasing through removal of the relevant embedding dimension. 
...
\begin{itemize}
    ...
    \item We perform comprehensive evaluations and provide comparisons with the previous work, showing that bidirectional imparting achieves a greater interpretability than alternatives without sacrificing performance. 
