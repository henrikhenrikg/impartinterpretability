...
by the following modified objective: \enote{hs}{maximize?}
\begin{align}
\begin{split}
\log ~ & \sigma ({\vec{\tilde{w}}_{j}}^T \vec{w}_{i}) + \sum_{t = 1}^{m} {\mathbb{E}}_{z_t \sim P_n(w)} \bigg[ log ~ \sigma({\vec{-\tilde{w}}_{z_t}}^T \vec{w}_{i}) \bigg] \\ 
...
Due to its advantages over word intrusion tests in terms of
scalability and reproducibility, here we adapt the
interpretability evaluation based on SEMCAT categories
\citep{senel18semanticStructure}. However, the
interpretability metric that is introduced in
\citet{senel18semanticStructure} and later improved in
\citet{senel18trInterpret}, cannot capture the difference
...
\enote{hs}{formula wrong?}
\enote{hs}{need to explain intuition}
