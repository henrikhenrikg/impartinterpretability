...
B^{g} = \mbox{corr}(b^{direct},b^{gd})\label{eq:bg}\\
     b^{gd}_p = 
     \begin{cases}
...
dimension. $B^{gd}$ and $B^g$ indicate how well the gender dimension
captures stereotypical bias, comparing to human judgments
and the direct bias measure
(Eq.\ \eqref{eq:direct_bias}).
...
\begin{table}
	...
	\caption{Accuracy and True Positive/Negative Rate
          ...
          task. B+S = \proposedmethod{}+strong imparting.}
	\label{tab:biosbias}
\end{table}
